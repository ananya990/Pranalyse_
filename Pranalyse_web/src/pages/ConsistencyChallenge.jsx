import React from "react";
import { useNavigate } from "react-router-dom";

function ConsistencyChallenge() {
  const navigate = useNavigate();

  return (
    <div className="bg-gradient-to-b from-violet-50 via-white to-violet-50 text-slate-800">

      {/* ================= HERO SECTION ================= */}
      <section
        className="relative h-[80vh] flex items-center"
        style={{
          backgroundImage:
            "url('https://images.unsplash.com/photo-1625662171040-8d196a082232?q=80&w=1770&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')",
          backgroundSize: "cover",
          backgroundPosition: "center"
        }}
      >
        {/* Light overlay */}
        <div className="absolute inset-0 bg-white/70 backdrop-blur-sm"></div>

        <div className="relative z-10 max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-10 items-center">

          {/* Left Content */}
          <div>
            <h1 className="text-4xl md:text-6xl font-extrabold mb-4 text-violet-800">
              Consistency Challenge
            </h1>
            <p className="text-xl text-slate-700 mb-6">
              Build healthy movement habits, one day at a time.
            </p>
            <p className="text-slate-600 max-w-xl">
              This challenge encourages short, daily exercises and stretches
              to improve your mobility, energy, and posture. Track your streaks
              and feel the difference over time.
            </p>
          </div>

          {/* Right CTA */}
          <div className="flex md:justify-end">
            <button
              onClick={() => navigate("/pose-detect")}
              className="px-8 py-4 text-lg font-semibold rounded-xl
                bg-violet-600 text-white
                hover:bg-violet-500
                shadow-lg hover:shadow-xl
                transition-all duration-300"
            >
              Start Now
            </button>
          </div>
        </div>
      </section>

      {/* ================= WHAT IS CONSISTENCY CHALLENGE ================= */}
      <section className="py-20 px-6 max-w-6xl mx-auto">
        <h2 className="text-3xl font-bold mb-6 text-violet-800">
          What is Consistency Challenge?
        </h2>
        <p className="text-slate-600 leading-relaxed max-w-4xl">
          The Consistency Challenge is a daily movement program designed to
          create lasting habits. Small exercises done consistently can
          significantly improve posture, flexibility, and overall well-being.
        </p>
      </section>

      {/* ================= COMMON HURDLES ================= */}
      <section className="py-20 px-6 bg-violet-50">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-bold mb-10 text-violet-800">
            Common Challenges People Face
          </h2>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              "Skipping days due to busy schedule",
              "Forgetting to stretch or move",
              "Losing motivation quickly",
              "Feeling too tired for exercise",
              "Not tracking progress",
              "Lack of guidance on proper movements"
            ].map((problem, index) => (
              <div
                key={index}
                className="bg-white p-6 rounded-xl shadow-sm border border-violet-100"
              >
                <p className="text-slate-700">{problem}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ================= DAILY MOVEMENTS ================= */}
      <section className="py-20 px-6 max-w-6xl mx-auto">
        <h2 className="text-3xl font-bold mb-10 text-violet-800">
          Daily Movements Included
        </h2>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            "Morning stretch routine",
            "Desk posture resets",
            "Neck & shoulder mobility",
            "Hip opening exercises",
            "Quick energizing breaks",
            "Breathing & mindfulness moments"
          ].map((exercise, index) => (
            <div
              key={index}
              className="bg-gradient-to-br from-violet-100 to-indigo-100
              p-6 rounded-xl shadow-sm border border-violet-200"
            >
              <p className="font-medium text-slate-700">{exercise}</p>
            </div>
          ))}
        </div>
      </section>

      {/* ================= BENEFITS ================= */}
      <section className="py-20 px-6 bg-violet-50">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-bold mb-10 text-violet-800">
            Benefits of Joining the Challenge
          </h2>

          <div className="grid md:grid-cols-2 gap-8">
            {[
              "Build long-term healthy habits",
              "Improve flexibility and mobility",
              "Boost energy and focus",
              "Reduce stiffness from prolonged sitting",
              "Increase motivation and discipline",
              "Track progress and celebrate streaks"
            ].map((benefit, index) => (
              <div
                key={index}
                className="flex items-start gap-4 bg-white p-6 rounded-xl shadow-sm"
              >
                <div className="text-violet-600 text-xl">✔</div>
                <p className="text-slate-700">{benefit}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ================= FINAL CTA ================= */}
      <section className="py-24 text-center px-6">
        <h2 className="text-4xl font-extrabold mb-6 text-violet-800">
          Start Your Streak Today
        </h2>
        <p className="text-slate-600 max-w-2xl mx-auto mb-10">
          Small daily movements done consistently lead to big long-term results.
        </p>
        <button
          onClick={() => navigate("/pose-detect")}
          className="px-10 py-4 text-lg font-semibold rounded-xl
            bg-violet-600 text-white
            hover:bg-violet-500
            shadow-lg hover:shadow-xl
            transition-all"
        >
          Start Consistency Challenge
        </button>
      </section>

    </div>
  );
}

export default ConsistencyChallenge;
